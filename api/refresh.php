...
function project_refresh($project_keyword, $pid) {
    $json = file_get_contents('https://newsapi.org/v2/everything?q='. prepare_get_string($project_keyword) . '&apiKey=4c5c8c29454c45c898f8fd3a11f2e707&language=en&pageSize=20&page=1');
    ...
    for ($i=0; $i < count($articles); $i++) { 
        ...
        if (! is_in_db($current_article["title"], $current_date)) {
            ...
            #console_log("article now inserted");
        } else {
            #console_log("article inserted already");
        }
    }
